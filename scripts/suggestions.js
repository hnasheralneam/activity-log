...
function generateSuggestions() {
   ...
   activities.forEach((el) => {
      ...
      if (labels.findIndex((name) => name == el.name) != -1) {
         data[labels.findIndex((name) => name == el.name)].push([el.time, el.start]);
      } else {
         labels.push(el.name);
         ranks.push(0);
         data.push([[el.time, el.start]]);
      }
   });
...
// Not sure what this means anymore - Days since activity should be calculated before adding to array item, by subtracting .01, multiplied by 1.1 for each consecutive week distant
